...
const LogIn = () => {
  ...
  const { mutate: loginMutation, isLoading: isLoading } = useLoginMutation();

  const handleSubmit = () => {
    if (validateRequiredField(formData?.username)) {
    ...
    loginMutation(formData, {
      onSuccess: (res) => {
        setValueInLocalStorage("accessToken", res?.token);
        setValueInLocalStorage("user", JSON.stringify(res?.user));
        setValueInLocalStorage("userId", res?.user?._id);
        setValueInLocalStorage("role", JSON.stringify(res?.user?.type));
        return router.push("/admin/manage-blog");
      },
    });
  };
      ...
      <Box className="max-w-[1280px] w-[100%] m-auto max-[1280px]:px-4">
        <Box className="w-[500px] max-[501px]:max-w-[500px] max-[501px]:w-[unset] shadow-md m-auto px-9 pb-8 pt-5 rounded-[4px] bg-[white]">
          <Image
            src={Logo?.src}
            alt={Logo?.src}
            className="w-[140px] object-cover mb-5"
          />
          ...
          <Box className="flex gap-4 flex-col">
            ...
            <Box className="flex justify-end">
              {/* <Link href={"/signup"} className="text-firstColorBg underline text-[14px]">
                Sign up
              </Link> */}
              <Link
                href={"/"}
                className="text-firstColorBg underline text-[14px]"
              >
                Return to Homepage
              </Link>
            </Box>
